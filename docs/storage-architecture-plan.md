...
- WebDAV 认证或写入失败时拒绝同步；缺失 ETag 与条件写入能力时进入兼容模式，清单更新必须先读取最新远端内容再合并写入，不得盲目覆盖。
...
验收：任何条件写入冲突都不会静默覆盖；新增内容双方都保留；删除不会被离线副本复活；服务器能力不足（认证或写入失败）时同步不会启用；兼容模式下的清单写入不得覆盖其他设备并发写入。
