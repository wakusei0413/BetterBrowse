...
import { IndexedDBManager, IDBStores, DB_VERSION } from './indexed-db.js';
...
export class MigrationManager {
  /**
   * 自愈修复：磁盘库存在但业务对象仓储缺失时，通过版本升级重建全部仓储并回填旧存储收纳数据。
   *
   * 背景：IndexedDB 拒绝用低于现存版本的号打开（VersionError），且"仅抬高版本号的裸打开"
   * 会产生一个版本号很高但没有任何对象仓储的空库——此后所有业务读写抛 NotFoundError，
   * 收纳读写全靠旧存储兜底，同步引擎则因仓储缺失陷入无限重试。
   * 修复方式：以 DB_VERSION 打开（高于残留版本时触发 upgradeneeded，_ensureSchema 幂等建表），
  ...
  static async repairMissingObjectStores() {
    ...
    try {
      // 打开（必要时升级建表）：磁盘残留版本低于 DB_VERSION 时触发 upgradeneeded 重建全部仓储
      ...
      // 数据架构尚未进入 IndexedDB 时代：旧数据由正式迁移管线（v5/v7/v8）负责回填，此处不抢
  ...
  /**
   * v6 修复迁移：清理 IndexedDB 中的两类历史异常条目
   ...
   * 操作幂等可重入：中断重跑不产生副作用，失败时版本停在 v5 下次重试。
  ...
  static async repairIndexedEntries() {
    ...
    try {
      return await IndexedDBManager.withWriteLock(async () => {
        ...
        console.info(
          `[MigrationManager] v6 修复完成：清理异常条目 ${toDelete.length} 条，规范化重写 ${toWrite.length} 条`
        );
        return true;
      });
    } catch (err) {
      console.warn('[MigrationManager] v6 条目修复执行异常，将于下次启动重试:', err);
      return false;
    }
  }

  /**
   * v7 迁移：将用户配置、域名跳转规则、自动备份与活动统计搬运至 IndexedDB
   * 写入使用无版本门控的直接仓储接口（此时 schema 仍为 6，StorageAdapter.set 仍指向 chrome.storage）。
   * 版本推进放在同一把写锁内，避免"已写主库但版本未切"窗口期把新写入打进旧存储。
   * @returns {Promise<boolean>}
   */
  static async migrateSettingsToIndexedDB() {
    const optedOut = (await StorageAdapter.getChrome(StorageKeys.IDB_OPTOUT, false)) === true;
    if (optedOut) {
      // 用户已显式回退：配置继续留在 chrome.storage，仅推进版本号
      return true;
    }
    if (!IndexedDBManager.isSupported()) {
      console.info('[MigrationManager] 当前环境不支持 IndexedDB，v7 配置迁移延后重试');
      return false;
    }
    try {
      return await IndexedDBManager.withWriteLock(async () => {
        ...
        if (storedConfig === undefined || storedRules === undefined) {
          console.warn('[MigrationManager] v7 配置迁移完整性校验未通过，将于下次启动重试');
          return false;
        }

        await StorageAdapter.setChrome(StorageKeys.IDB_SETTINGS_MIGRATED_AT, Date.now());
        // 版本推进必须在同一把写锁内完成，避免并发配置写入落到旧存储后被主库覆盖
      ...
      console.warn('[MigrationManager] v7 配置迁移执行异常，将于下次启动重试:', err);
      return false;
    }
  }

  /**
   * v8 迁移：为 WebDAV 同步准备本地元数据（幂等、可中断重跑）
   ...
   * 全程持跨上下文写锁，失败时版本停在 v7 下次重试。
   * @returns {Promise<boolean>}
   */
  static async prepareSyncMetadata() {
    if (!IndexedDBManager.isSupported()) {
      console.info('[MigrationManager] 当前环境不支持 IndexedDB，v8 同步元数据准备延后重试');
      return false;
    }
    try {
      return await IndexedDBManager.withWriteLock(async () => {
        ...
        if (stats && typeof stats === 'object' && !Array.isArray(stats)) {
          ...
          if (dropped > 0 || Object.keys(converted).length !== Object.keys(stats).length) {
            ...
            console.info(`[MigrationManager] v8 已将活跃度转换为按 pageId 存储（丢弃无法映射的旧记录 ${dropped} 条）`);
          }
        }
        ...
        if (!clockCheck?.value?.deviceId) {
          console.warn('[MigrationManager] v8 同步元数据完整性校验未通过，将于下次启动重试');
          return false;
        }
        console.info('[MigrationManager] v8 同步元数据准备完成（本机时钟、实体字段与活跃度 pageId 化）');
        return true;
      });
    } catch (err) {
      console.warn('[MigrationManager] v8 同步元数据准备异常，将于下次启动重试:', err);
      return false;
    }
  }
  ...
  static async rollbackFromIndexedDB() {
    ...
    try {
      ...
      return await IndexedDBManager.withWriteLock(async () => {
        ...
        // 同步导回 v7 配置主库，保证回退后 chrome.storage 仍是完整数据源
